...
public class TargetSelector {
	...
	public TargetSelector(ParticleAnalysisReport[] reports, int selectedTarget,
	...
	private void evaluateTargets() {
		...
		for (int i = 0; i < m_targets.length; i++) {
			...
			// The "best" target is based on the composite score. We may want to
			// include a term
			// in the composite score that includes the distance from the
			// current target to the
			// selected target.
			if (curTarget.isValid()) {
				if (m_bestTarget == null
						|| (curTarget.getCompositeScore() < m_bestTarget
								.getCompositeScore())) {
		...
		if (rightIndex != -1) {
			m_targets[rightIndex].setTargetLocations(Target.RIGHT,
					m_selectedTarget);
		}
		if (leftIndex != -1) {
			m_targets[leftIndex].setTargetLocations(Target.LEFT,
					m_selectedTarget);
		}
		if (topIndex != -1) {
			m_targets[topIndex]
					.setTargetLocations(Target.TOP, m_selectedTarget);
		}
		if (bottomIndex != -1) {
			m_targets[bottomIndex].setTargetLocations(Target.BOTTOM,
					m_selectedTarget);
		}
	}
	...
	public double getAverageHorizontalAngleToSelectedTargetDeg() {
		...
		for (int i = 0; i < m_targets.length; i++) {
			if (m_targets[i].isValid()) {
				horizontalAngle += m_targets[i]
						.getHorizontalAngleToSelectedTargetDeg();
				numValidTargets++;
			}
		}
		if (numValidTargets > 0) {
