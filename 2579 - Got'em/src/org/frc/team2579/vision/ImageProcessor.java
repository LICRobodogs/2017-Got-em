package org.frc.team2579.vision;
...
public class ImageProcessor {
	...
	public static final double TARGET_ASPECT_RATIO = TARGET_HEIGHT_FT
			/ TARGET_WIDTH_FT;
	...
	public static final double tanHalfFOV = Math.tan(Math
			.toRadians(CAMERA_FOV_HORIZONTAL_ANGLE / 2));

	public static final double MINIMUM_VALID_COMPOSITE_SCORE = 6;

	public Image processedImage = NIVision.imaqCreateImage(ImageType.IMAGE_U8,
			0);
	...
	public TargetInfo findBestTarget(Image cameraImage, boolean includeOverlays) {
		...
		try {
			NIVision.imaqColorThreshold(processedImage, cameraImage, 1,
					NIVision.ColorMode.HSL, TARGET_HUE_RANGE, TARGET_SAT_RANGE,
					TARGET_LUM_RANGE);
			...
			// NIVision.StructuringElement element = new
			// NIVision.StructuringElement();
			// NIVision.imaqSizeFilter(filteredImage, processedImage, 1,
			// NUM_SMALL_OBJECT_EROSIONS, NIVision.SizeType.KEEP_LARGE,
			// element);
			// if (filteredImage == null) {
			// return null;
			// }

			// The measurements are flipped because the image is rotated 90
			// degrees
			...
			for (int particleIndex = 0; particleIndex < numParticles; particleIndex++) {
				double rectWidth = NIVision.imaqMeasureParticle(processedImage,
						particleIndex, 0,
						NIVision.MeasurementType.MT_BOUNDING_RECT_HEIGHT);
				double rectHeight = NIVision.imaqMeasureParticle(
						processedImage, particleIndex, 0,
						NIVision.MeasurementType.MT_BOUNDING_RECT_WIDTH);
				double rectArea = NIVision.imaqMeasureParticle(processedImage,
						particleIndex, 0, NIVision.MeasurementType.MT_AREA);
				double rectMomentYY = NIVision.imaqMeasureParticle(
						processedImage, particleIndex, 0,
						NIVision.MeasurementType.MT_NORM_MOMENT_OF_INERTIA_XX);
				double rectMomentXX = NIVision.imaqMeasureParticle(
						processedImage, particleIndex, 0,
						NIVision.MeasurementType.MT_NORM_MOMENT_OF_INERTIA_YY);

				double currentCompositeScore = getCompositeScore(rectArea,
						rectWidth, rectHeight, rectMomentXX, rectMomentYY);
				if (bestTargetIndex == -1
						|| currentCompositeScore < bestCompositeScore) {
			...
			double rectTop = NIVision.imaqMeasureParticle(processedImage,
					bestTargetIndex, 0,
					NIVision.MeasurementType.MT_BOUNDING_RECT_LEFT);
			double rectLeft = NIVision.imaqMeasureParticle(processedImage,
					bestTargetIndex, 0,
					NIVision.MeasurementType.MT_BOUNDING_RECT_TOP);
			double rectWidth = NIVision.imaqMeasureParticle(processedImage,
					bestTargetIndex, 0,
					NIVision.MeasurementType.MT_BOUNDING_RECT_HEIGHT);
			double rectHeight = NIVision.imaqMeasureParticle(processedImage,
					bestTargetIndex, 0,
					NIVision.MeasurementType.MT_BOUNDING_RECT_WIDTH);
			...
			double offsetAngle = Math.atan(widthOffsetPixels
					/ focalDistancePixels);

			double imageWidthFt = (double) imageWidth * TARGET_WIDTH_FT
					/ rectWidth / Math.cos(offsetAngle);
			double cameraDistanceWidthFt = Math.cos(Math
					.toRadians(CAMERA_AIM_VERTICAL_ANGLE))
					* imageWidthFt
					/ 2.0
					/ tanHalfFOV;

			// Calculate the angle from the center of the image to the selected
			// target
			double targetOffsetFt = imageWidthFt * widthOffsetPixels
					/ (double) imageWidth + CAMERA_OFFSET_FT;
			double angleToTargetDeg = Math.atan2(targetOffsetFt,
					cameraDistanceWidthFt) * 180.0 / Math.PI;

			if (includeOverlays) {
				NIVision.Rect rect = new NIVision.Rect((int) rectLeft,
						(int) rectTop, (int) rectWidth, (int) rectHeight);
				NIVision.imaqDrawShapeOnImage(cameraImage, cameraImage, rect,
						DrawMode.DRAW_VALUE, ShapeMode.SHAPE_RECT, 65000);

				int targetYCoord = (int) (rectLeft + rectWidth / 2);
				NIVision.Point startPoint = new NIVision.Point(0, targetYCoord);
				NIVision.Point endPoint = new NIVision.Point(imageSize.width,
						targetYCoord);
				NIVision.imaqDrawLineOnImage(cameraImage, cameraImage,
						DrawMode.DRAW_VALUE, startPoint, endPoint, 65000);
			}

			return new TargetInfo(cameraDistanceWidthFt, angleToTargetDeg,
					bestCompositeScore);
		} catch (Exception e) {
			System.err
					.println("An error occurred in vision processing.  Message = "
							+ e.getMessage());
			return null;
		}
	}
	...
	private double getCompositeScore(double area, double width, double height,
			double momentXX, double momentYY) {
		return Math.abs(getRectangleScore(area, width, height))
				+ Math.abs(getXXRatioScore(momentXX))
				+ Math.abs(getYYRatioScore(momentYY))
				+ Math.abs(getAspectRatioScore(width, height));
	}
	...
	public class TargetInfo {
		...
		public TargetInfo(double distanceToTargetFt, double angleToTargetDeg,
