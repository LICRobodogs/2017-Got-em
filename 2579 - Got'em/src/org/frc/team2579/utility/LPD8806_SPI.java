...
public class LPD8806_SPI extends SensorBase {
	 ...
	 */
	public LPD8806_SPI() {
		// Don't really need a chip select, but for now the SPIDevice class
		// requires one
		spi = new SPI(Port.kOnboardCS1);
		spi.setChipSelectActiveHigh();

		// Based on the Adafruit Arduino C++ code for the LPD8806 this is
		// correct
		spi.setMSBFirst();

		// Based on the Adafruit Arduino C++ code for the LPD8806 they use
		// SPI_MODE0 which is:
		...
		// Based on the Adafruit Arduino C++ code for the LPD8806 they use
		// setClockDivider(SPI_CLOCK_DIV8) which
		// sets the SPI clock divider relative to the system clock. On AVR based
		// Arduino boards,
		// the dividers available are 2, 4, 8, 16, 32, 64 or 128. The default
		// setting is SPI_CLOCK_DIV4,
		// which sets the SPI clock to one-quarter the frequency of the system
		// clock (4 Mhz for the boards at 16 MHz).
		// So SPI_CLOCK_DIV8 will set it at 2Mhz. The notes in the LPD8806 code
		// say, "Although the LPD8806 should,
		// in theory, work up to 20MHz, the unshielded wiring from the Arduino
		// is more susceptible to interference.
		// Experiment and see what you get.
		//
		// The max clock rate from SPIDevice is 76628 hz which is 0.076628Mhz...
		// 26x slower. Not sure if this is OK or if we
		// are comparing apples to apples. Update: 76628 hz seems to work fine
		spi.setClockRate(76628);
	...
	private int getColor(int r, int g, int b) {
		return ((g | 0x80) << 16) | ((r | 0x80) << 8) | b | 0x80;
	}

	private void sendLatchReset() {
		for (int i = 0; i < latchLen; i++) {
			spi.write(new byte[] { 0 }, 8);
		}
	}

	public void sendColors() {
		// spi.transaction(colors, colorBits);
		sendLatchReset();
		sendLatchReset();
	}
}