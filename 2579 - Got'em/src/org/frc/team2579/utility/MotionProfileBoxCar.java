...
public class MotionProfileBoxCar {
	...
	public MotionProfileBoxCar(double startDistance, double targetDistance,
			double maxVelocity, double itp) {
		this(startDistance, targetDistance, maxVelocity, itp, DEFAULT_T1,
				DEFAULT_T2);
	}

	public MotionProfileBoxCar(double startDistance, double targetDistance,
			double maxVelocity, double itp, double t1, double t2) {
	...
	private void initializeProfile() {
		// t4 is the time in ms it takes to get to the end point when at max
		// velocity
		...
		// In the case where t4 is less than the accel times, we need to adjust
		// the
		// accel times down so the filters works out. Lots of ways to do this
		// but
		...
		// Adjust max velocity so that the end point works out to the correct
		// position.
	...
	public MotionProfilePoint getNextPoint(MotionProfilePoint point) {
		...
		point.position = previousPosition + (point.velocity + previousVelocity)
				/ 2 * itp / 1000;
	...
	public static void main(String[] args) {
		...
		MotionProfileBoxCar mp = new MotionProfileBoxCar(0, 96, 30, 10, 200,
				100);
		...
		while (mp.getNextPoint(point) != null) {
			System.out.println(point.time + ", " + point.position + ", "
					+ point.velocity + ", " + point.acceleration);
		}

		long deltaTime = System.nanoTime() - startTime;
		System.out.println("Time Box Car = " + (double) deltaTime * 1E-6
				+ " ms");
	}
}