...
public class MotionProfileBoxCarArray {
	...
	public MotionProfileBoxCarArray(double targetDistance, double maxVelocity,
			double itp) {
	...
	public MotionProfilePoint[] getProfile() {
		...
		for (int i = 0; i < numITP; i++) {
			...
			currentPoint.position = previousDistance
					+ (currentPoint.velocity + previousVelocity) / 2 * itp
					/ 1000;
			currentPoint.acceleration = (currentPoint.velocity - previousVelocity)
					/ itp * 1000;
	...
	public static void main(String[] args) {
		...
		// MotionProfileBoxCarArray mp = new MotionProfileBoxCarArray(4, 10,
		// 10);
		...
		System.out.println("Time Box Car Array = " + (double) deltaTime * 1E-6
				+ " ms");
	}
