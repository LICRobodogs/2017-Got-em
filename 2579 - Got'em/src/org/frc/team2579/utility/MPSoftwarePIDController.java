...
public class MPSoftwarePIDController {
	public static enum MPSoftwareTurnType {
	...
	protected double prevError = 0.0; // the prior error (used to compute
										// velocity)
	protected double totalError = 0.0; // the sum of the errors for use in the
										// integral calc

	public MPSoftwarePIDController(long periodMs, PIDParams pidParams,
	...
	public void setMPTurnTarget(double startAngleDeg, double targetAngleDeg,
			...
			MPSoftwareTurnType turnType, double trackWidth) {
		...
		mp = new MotionProfileBoxCar(startAngleDeg, targetAngleDeg,
				maxTurnRateDegPerSec, periodMs, t1, t2);
	...
	public boolean controlLoopUpdate(double currentGyroAngle) {
		...
		double output = pidParams.kP * error + pidParams.kI * totalError
				+ pidParams.kD * (error - prevError) + pidParams.kA
				* mpPoint.acceleration + pidParams.kV * mpPoint.velocity;
		prevError = error;
