 ...
 * @author rhhs
 */
public abstract class ExtraTwoTimeoutCommand extends Command {
	...
	protected synchronized final void startExtraTimeout1(double seconds) {
		if (seconds < 0) {
			throw new IllegalArgumentException(
					"Seconds must be positive.  Given:" + seconds);
		}
	...
	public synchronized final double timeSinceExtraInitialized1() {
		return m_startExtraTime1 < 0 ? 0 : Timer.getFPGATimestamp()
				- m_startExtraTime1;
	}

	public synchronized final double timeSinceExtraInitialized2() {
		return m_startExtraTime2 < 0 ? 0 : Timer.getFPGATimestamp()
				- m_startExtraTime2;
	}
	...
	protected synchronized boolean isExtraTimedOut1() {
		return m_extraTimeout1 != -1
				&& timeSinceExtraInitialized1() >= m_extraTimeout1;
	}

	protected synchronized boolean isExtraTimedOut2() {
		return m_extraTimeout2 != -1
				&& timeSinceExtraInitialized2() >= m_extraTimeout2;
	}
